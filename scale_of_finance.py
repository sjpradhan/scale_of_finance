...
def main():
    ...
    with col1:
        # File uploader
        uploaded_file = st.file_uploader(":orange[Choose an Excel file] 📁", type=["xlsx", "xls","csv","pdf","txt"])
        ...
        with col2:
            ...
            def load_district_data():
                ...
                district_df = district_df[
                    ["District LGD Code", "District Name (In English)", "District Name (In Local language)",
                     "Hierarchy"]].rename(columns={"District LGD Code": "District Code",
                                                   "District Name (In English)": "District Name",
                                                   "District Name (In Local language)": "District Local Name",
                                                   "Hierarchy": "State Name"})

                district_df['State Name'] = district_df['State Name'].str.extract(r'^(.*)\(State\)$')[0]
            ...
            if search_term:
                filtered_records = district_df[
                    district_df["District Name"].str.contains(search_term, case=False) |
                    district_df["District Code"].astype(str).str.contains(search_term) |
                    district_df["State Name"].str.contains(search_term, case=False)
                    ]
